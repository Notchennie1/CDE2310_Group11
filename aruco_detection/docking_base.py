...
class DockingBase(Node):
    ...
    def active_cb(self, msg):
        ...
        if self.is_active:
            ...
            self.get_logger().info('Activated — arcing to align in front of marker...')
        else:
    ...
    def drive_callback(self):
        ...
        # ── STEP 1: Arc drive to get directly in front of the marker ──
        if self.state == 'align_front':
            ...
            if marker_age > self.marker_timeout:
                self.cmd_pub.publish(Twist())
                self.get_logger().warn('Align front: marker lost — waiting...')
                return
            ...
            if abs(x) < self.lateral_threshold:
                self.cmd_pub.publish(Twist())
                self.get_logger().info(
                    f'In front of marker at x={x:.3f}m z={z:.3f}m — turning to face...'
                )
            ...
            cmd.linear.x = 0.03   # slow forward arc
            self.cmd_pub.publish(cmd)
            self.get_logger().info(f'Align front: x={x:.3f}m z={z:.3f}m')
        ...
        # ── STEP 2: Rotate in place to face the marker squarely ──
            ...
            if marker_age > self.marker_timeout:
                self.cmd_pub.publish(Twist())
                self.get_logger().warn('Turn face: marker lost — waiting...')
                return
